...
class API {
    ...
    //
    // 呼叫RT server, 回傳 [ {id:'2330.TW', name:'..', .. }, { } ]
    //
    rtQuote(req, res) {
    ...
    _parseQuoteXML(text) {
      ...
      for (idx = 0; idx < nodes.length; idx++) {
        ...
        let symbol = {
          symbolId: id + ".TW",       // TODO: 這裡只支援台股
          name: this.symbolSvc.getName(id),
          dp: 2,
          date: this._getNumberField(node, "Date"),
          preClose: this._getPriceField(node, "PC"),
          preVolume: this._getNumberField(node, "PV"),
          ...
          tick: {
            ...
            inout: this._getNumberField(node, "IO")
          },
          bid: [
